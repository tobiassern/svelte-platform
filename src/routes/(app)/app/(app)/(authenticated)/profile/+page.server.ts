...
export const load: PageServerLoad = async (event) => {
	...
	const profile_form = await superValidate(
...
export const actions: Actions = {
	...
	'delete-session': async (event) => {
		...
		if (sessionId === event.locals.session?.id) {
			return fail(400, {
				message: 'Not possible to delete current session. Use logout functionality instead.'
			});
		}

		await event.locals.db
			.delete(sessions_table)
			.where(and(eq(sessions_table.userId, user.id), eq(sessions_table.id, sessionId)));
