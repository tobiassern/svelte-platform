<script lang="ts">
	...
	const { form, errors, enhance, submitting, delayed } = superForm(activate_invite_link_form, {
		...
		onUpdated: ({ form }) => {
			console.log(form);
			if (form.valid) {
				toast.success(`Invite link ${form.data.inviteLinkActive ? 'activated' : 'inactivated'}`);
			}
		}
	});

	$: console.log($form.inviteLinkActive);
</script>
...
>
	...
	<Switch
		...
		name="inviteLinkActive"
		aria-label="Invite Link Active"
		value="active"
		bind:checked={$form.inviteLinkActive}
		disabled={$submitting}
	/>
</form>
