...
import { update_site_general_information_schema, update_subdomain_schema, update_site_cover_image_schema } from "$lib/schemas/form";
...
export const actions: Actions = {
    ...
    'update-cover-image': async (event) => {
        ...
        const cover_image_form = await superValidate(event.request, zod(update_site_cover_image_schema));
        ...
        if (cover_image_form.data.cover_image) {
            ...
            await event.locals.db.update(sites_table).set({ cover_image_url: blob.url }).where(eq(sites_table.id, site.id))
        } else {
            await event.locals.db.update(sites_table).set({ cover_image_url: null }).where(eq(sites_table.id, site.id))
        }
    ...
    'update-subdomain': async (event) => {
        ...
        await event.locals.db.update(sites_table).set(subdomain_form.data).where(eq(sites_table.id, site.id));
