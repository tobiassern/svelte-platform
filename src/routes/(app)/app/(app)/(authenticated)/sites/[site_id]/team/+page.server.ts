...
export const load: PageServerLoad = async (event) => {
    ...
    if (!site.invite_link_id) {
        ...
        const [result] = await event.locals.db.update(sites_table).set({ invite_link_id: createId() }).where(eq(sites_table.id, site.id)).returning();
    ...
    return {
        site,
        members: await event.locals.db.select({ id: users_table.id, name: users_table.name, avatarUrl: users_table.avatarUrl, email: users_table.email }).from(siteMembers).where(eq(siteMembers.site_id, site.id)).leftJoin(users_table, eq(users_table.id, siteMembers.user_id)),
...
export const actions: Actions = {
    'activate-invite-link': async (event) => {
        ...
        await event.locals.db.update(sites_table).set({ invite_link_active: activate_invite_link_form.data.invite_link_active ? true : false }).where(eq(sites_table.id, site.id))
    ...
    'remove-member': async (event) => {
        ...
        const deletedMember = await event.locals.db.delete(siteMembers).where(eq(siteMembers.user_id, userId)).returning();
    ...
    'reroll-invite-link': async (event) => {
        const site = await isSiteMember(event);
        await event.locals.db.update(sites_table).set({ invite_link_id: createId() }).where(eq(sites_table.id, site.id)).returning();
        return { success: true }
    }
}