...
export const GET: RequestHandler = async (event) => {
	...
	try {
		...
		const existingUser = await event.locals.db
			...
			.where(
				and(
					eq(oauth_accounts_table.providerId, 'github'),
					eq(oauth_accounts_table.providerUserId, githubUser.id)
				)
			);
		if (existingUser[0]) {
			...
			await event.locals.db
				.insert(oauth_accounts_table)
				.values({ providerId: 'github', providerUserId: githubUser.id, userId: userId });
